...
Data Gathering: Read and preprocess the data. FAQ retrived

Embedding: Use Sentence-BERT to generate dense vector embeddings for both the document and user queries.
Vector DB: Store document embeddings in a FAISS or Pinecone index for fast retrieval.
User Query: Convert the user’s query into an embedding.
Retrieve Top-k: Use vector search to find the top-k most relevant document chunks.
